import numpy as np
import cv2

image = cv2.imread("/home/ebuka/Documents/Mastering Computer vision/images/someshapes.jpg")
gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

cv2.imshow('Identifying Shapes', image)
cv2.waitKey(0)

ret, thresh = cv2.threshold(gray, 127, 255, 1)

im2, contours, hierarchy = cv2.findContours(thresh.copy(), cv2.RETR_LIST
                                            , cv2.CHAIN_APPROX_NONE)

for cnt in contours:
    approx = cv2.approxPolyDP(cnt, 0.01*cv2.arcLength(cnt, True), True)

    if len(approx) == 3:
        shape_name = "Triangle"
        cv2.drawContours(image, [cnt], 0, (0,255,0), -1)

        M = cv2.moments(cnt)
        cx = int(M['m10'] / M['m00'])
        cy = int(M['m01'] / M['m00'])
        cv2.putText(image, shape_name, (cx - 55, cy), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1)

    elif len(approx) == 4:
        x,y,w,h = cv2.boundingRect(cnt)
        M = cv2.moments(cnt)
        cx = int(M['m10'] / M['m00'])
        cy = int(M['m01'] / M['m00'])

        if abs(w-h) <= 3:
            shape_name = "Square"
            cv2.drawContours(image, [cnt], 0, (0,127,255), -1)
            cv2.putText(image, shape_name, (cx - 50, cy), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1)
        else:
            shape_name = "Rectangle"
            cv2.drawContours(image, [cnt], 0, (0,0,255), -1)
            cv2.putText(image, shape_name, (cx - 75, cy), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1)

    elif len(approx) == 10:
        shape_name = "Star"
        M = cv2.moments(cnt)
        cx = int(M['m10'] / M['m00'])
        cy = int(M['m01'] / M['m00'])
        cv2.drawContours(image, [cnt], 0, (255,250,0), -1)
        cv2.putText(image, shape_name, (cx -30, cy), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1)
        
    elif len(approx) >= 15:
        shape_name = "Circle"
        M = cv2.moments(cnt)
        cx = int(M['m10'] / M['m00'])
        cy = int(M['m01'] / M['m00'])
        cv2.drawContours(image, [cnt], 0, (0,255,255), -1)
        cv2.putText(image, shape_name, (cx - 40, cy + 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 1)
    cv2.imshow('Identify Shapes', image)
    cv2.waitKey(0)

cv2.destroyAllWindows()
        
